...
def get_feature_target(
   ...
   # Verificar las columnas en app_train
    print("Columns in app_train:", app_train.columns)
    
    # Asegúrate de que la columna 'TARGET' existe en app_train
    if 'TARGET' not in app_train.columns:
        raise ValueError("The 'TARGET' column is missing from app_train.")
    ...
    y_test = app_test["TARGET"] if "TARGET" in app_test.columns else None
...
def get_train_val_sets(
    ...
    # Shuffle the data (see `shuffle` parameter in `train_test_split`).
     ...
     # Validar que X_train y y_train no sean None ni estén vacíos
    ...
    # Verificar que el número de filas de X_train y y_train sea igual
    ...
    # Verificar que la partición haya sido exitosa y que los resultados no sean vacíos
    if X_train.empty or X_val.empty or y_train.empty or y_val.empty:
        raise ValueError("Training or validation sets are empty after the split.")
